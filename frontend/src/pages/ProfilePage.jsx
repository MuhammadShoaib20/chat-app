...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await api.put('/api/users/profile', { 
        username, 
        avatar // base64 string or existing URL
      });
      ...
      toast.error(
        ...
        'Update failed. This might be a backend issue – avatar upload may not be supported yet.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center px-4 py-8">
      <div className="max-w-md w-full bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Select a new image (max 2MB). If update fails, the backend might not support avatar changes yet.
            </p>
          </div>
