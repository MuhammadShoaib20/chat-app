...
const ChatWindow = ({ conversationId, conversation, onOpenInfo, onOpenSidebar }) => {
  ...
  const displayName =
    conversation?.isGroup
      ? conversation.name
      : otherParticipant?.userId?.username || 'User';
  ...
  useEffect(() => {
    ...
    markAsRead({ conversationId, messageIds: unread.map((m) => m._id) }).catch(
      (e) => console.error('Mark read error:', e)
    );
    ...
    <div className="flex flex-col h-full max-h-screen bg-white dark:bg-gray-950 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="flex-shrink-0 z-20 flex items-center justify-between px-3 py-3 md:px-5 md:py-3.5 bg-white/90 dark:bg-gray-950/90 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 gap-2 transition-colors duration-300">
        ...
        <div className="flex items-center min-w-0 flex-1 gap-2.5">
          ...
          {onOpenSidebar && (
            <button
              ...
              className="lg:hidden p-2 -ml-1 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200 active:scale-90 flex-shrink-0"
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          {conversation?.isGroup && (
            <button
              onClick={onOpenInfo}
              className="p-2 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200 active:scale-90"
        ...
        <div className="flex-shrink-0 px-4 py-3 bg-white/95 dark:bg-gray-950/95 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 z-10 shadow-sm transition-colors duration-300">
          <div className="relative max-w-2xl mx-auto">
            ...
            {searchQuery && (
              ...
              >
                <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" strokeLinecap="round" /></svg>
              </button>
            )}
          </div>
      ...
      {showScrollBtn && (
        <button
          ...
          className="absolute bottom-24 right-5 z-30 w-9 h-9 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full shadow-lg flex items-center justify-center text-blue-600 dark:text-blue-400 hover:shadow-xl hover:scale-110 active:scale-95 transition-all duration-200"
