...
const ChatWindow = ({
  ...
  // Auto-scroll when near bottom after new messages
  useEffect(() => {
  ...
  if (!conversationId) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-950 p-6 text-center animate-in fade-in duration-500">
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen bg-white dark:bg-gray-950 relative overflow-hidden">
      ...
      <div className="flex-shrink-0 z-20 flex items-center justify-between px-4 py-3 md:px-5 md:py-4 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 gap-2">
        ...
        <div className="flex items-center gap-1 md:gap-1.5 flex-shrink-0">
          ...
          {conversation?.isGroup && (
            <button onClick={onOpenInfo} className="p-2 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all active:scale-90" aria-label="Group info">
              <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      {/* ── Search Overlay ── */}
      {showSearch && (
        <div className="absolute top-[64px] left-0 right-0 p-3 bg-white/95 dark:bg-gray-950/95 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 z-30 animate-in slide-in-from-top duration-200 shadow-lg">
          ...
          {searchResults.length > 0 && (
            <div className="mt-2 max-w-2xl mx-auto max-h-48 overflow-y-auto rounded-xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-xl custom-scrollbar">
              {searchResults.map((msg) => (
                <div key={msg._id} className="p-3 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors border-b last:border-0 border-gray-50 dark:border-gray-800" onClick={() => setShowSearch(false)}>
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-2 md:px-5 bg-gray-50 dark:bg-gray-950 custom-scrollbar flex flex-col"
      >
        {loadingInitial ? (
          <div className="space-y-6 p-2"><Skeleton count={4} height={70} borderRadius={16} /></div>
        ) : (
          <div className="flex flex-col min-h-full">
            <div className="flex-1" /> {/* Pushes content to bottom when few messages */}
            ...
            {hasMore && (
              <div className="flex justify-center py-4">
                <button
                  ...
                  disabled={loading}
                  className="text-[10px] font-bold uppercase tracking-wider bg-white dark:bg-gray-800 text-blue-600 dark:text-blue-400 px-4 py-1.5 rounded-full shadow-sm border border-gray-100 dark:border-gray-700 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                >
              ...
              {messagesWithDateHeaders.map((item, index) => (
                item.type === 'date' ? (
                  <div key={`date-${index}`} className="flex justify-center my-6">
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 px-3 py-1 rounded-lg">
                  ...
                  <MessageBubble
                    ...
                    showAvatar={conversation?.isGroup && item.message.sender?._id !== user?._id}
                  />
                )
              ))}

              {typingUsers.length > 0 && (
                <div className="flex items-center gap-2.5 px-2 py-1 animate-in fade-in slide-in-from-left-2 duration-300 mt-2">
                  <div className="flex gap-1 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl rounded-tl-none px-3 py-2 shadow-sm">
                    <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                    <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                    <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                  </div>
                  <span className="text-[11px] font-medium text-gray-400 italic">
                    {typingUsers.length === 1 ? `${typingUsers[0].username} is typing...` : 'People are typing...'}
                  </span>
                </div>
              )}
      ...
      {(blockStatus.hasBlocked || blockStatus.isBlockedBy) && (
        <div className="flex-shrink-0 px-4 py-2 bg-red-50 dark:bg-red-900/10 border-t border-red-100 dark:border-red-900/20">
          <p className="text-[10px] text-center text-red-500 dark:text-red-400 font-bold uppercase tracking-widest">
            {blockStatus.hasBlocked ? 'You have blocked this user' : 'This user has blocked you'}
          </p>
        </div>
      )}

      {/* ── Input Area ── */}
      <div className="flex-shrink-0 bg-white dark:bg-gray-950">
        <MessageInput
