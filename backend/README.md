...
| Redis            | Caching and socket.io adapter (opt) |
...
# Redis (optional – example using Redis Cloud)
REDIS_URL=redis://:your_password@1.ec2.cloud.redislabs.com:18465
...
- Redis: start `redis-server` (or use your Redis Cloud URL, no need to run locally).
...
| `REDIS_URL`             | Redis connection URL (e.g., `redis://localhost:6379`) | No   |
| `VAPID_SUBJECT`         | Email address for VAPID (push notifications)     | No       |
...
All endpoints are prefixed with `/api`. Authentication is required for most endpoints (JWT token via `Authorization: Bearer <token>` header).
...
| Method | Endpoint                     | Description                           | Auth |
|--------|------------------------------|---------------------------------------|------|
| GET    | `/`                          | Get all conversations for the user    | Yes  |
| POST   | `/`                          | Create a new conversation (1:1 or group) | Yes |
| GET    | `/:id`                       | Get conversation details              | Yes  |
| PUT    | `/:id`                       | Update group info (name/avatar)       | Yes  |
| DELETE | `/:id`                       | Delete conversation                   | Yes  |
| POST   | `/:id/participants`          | Add participants (group only)         | Yes  |
| DELETE | `/:id/participants/:userId`  | Remove participant                    | Yes  |
| POST   | `/:id/hide`                  | Hide conversation for current user    | Yes  |
...
| POST   | `/`                                 | Send a new message (REST)  | Yes  |
...
| GET    | `/search`                           | Search messages in conversation | Yes |
...
> **Note:** For real-time messaging, use Socket.io events (see below). The REST endpoints are fallbacks and for fetching historical data.

---

## 🔌 Socket.io Events

The server uses Socket.io for real-time communication. Events are handled in `src/sockets/index.js`.

### Client → Server Events

| Event                  | Payload                                | Description                          |
|------------------------|----------------------------------------|--------------------------------------|
| `join-conversation`    | `conversationId`                       | Join a room for a specific conversation |
| `leave-conversation`   | `conversationId`                       | Leave conversation room              |
| `typing-start`         | `{ conversationId }`                   | Notify others that user is typing    |
| `typing-stop`          | `{ conversationId }`                   | Notify stop typing                   |
| `send-message`         | `{ conversationId, content, type, mediaUrl }` | Send a new message           |
| `edit-message`         | `{ messageId, content }`               | Edit an existing message             |
| `delete-message`       | `{ messageId }`                        | Soft delete a message                |
| `add-reaction`         | `{ messageId, emoji }`                 | Toggle reaction on a message         |

### Server → Client Events

| Event                  | Payload                                | Description                          |
|------------------------|----------------------------------------|--------------------------------------|
| `new-message`          | `Message` object                       | Broadcast new message to conversation room |
| `message-updated`      | `Message` object                       | Notify when a message is edited/deleted |
| `messages-read`        | `{ userId, messageIds, conversationId }` | Notify when messages are read      |
| `user-typing`          | `{ userId, username, conversationId }` | Typing indicator                    |
| `user-stopped-typing`  | `{ userId, conversationId }`           | Stop typing indicator               |
| `conversation-updated` | `Conversation` object                  | Notify participants of conversation changes |
| `participant-added`    | `{ conversationId, participants }`     | Notify group of new members         |
| `participant-removed`  | `{ conversationId, userId }`           | Notify group of removed member      |
| `user-online`          | `userId`                               | Notify when a user comes online     |
| `user-offline`         | `userId`                               | Notify when a user goes offline     |
| `online-users`         | `[userId]`                             | List of currently online users (sent on connection) |
...
The backend is deployed on **Render**. You can deploy to any Node.js hosting service (Render, Railway, Heroku, etc.).
...
5. Add all environment variables from your `.env` file (make sure to include `MONGO_URI`, `JWT_SECRET`, `CLIENT_URL`, etc.).
6. Click **Deploy**.

> ⚠️ **Important:** 
> - Ensure your MongoDB Atlas connection string includes the database name and correct credentials.
> - If using Redis, provide the `REDIS_URL` (as you have in your environment).
> - Set `NODE_ENV=production` for production optimizations.
> - Update `CLIENT_URL` to your actual frontend domain(s).
